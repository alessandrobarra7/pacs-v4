...
export default function UnitDoctorsTab({ unitId }: UnitDoctorsTabProps) {
  ...
  const handleAdd = () => {
    ...
    addDoctor.mutate({ unitId, doctorUserId: parseInt(selectedDoctorId, 10) });
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-sm text-muted-foreground bg-blue-50 border border-blue-100 rounded-md px-3 py-2">
        ...
        <span>Médicos vinculados têm acesso aos exames desta unidade. Preços e vigências são configurados exclusivamente no módulo Financeiro.</span>
      </div>
              ...
              {linkedDoctors.map(doc => (
                <TableRow key={doc.id} className="hover:bg-gray-50">
                  ...
                  <TableCell className="py-2.5 text-right">
                    <Button variant="ghost" size="sm" onClick={() => handleRemove(doc.id)} disabled={removeDoctor.isPending} className="text-red-600 hover:text-red-700 hover:bg-red-50 gap-1 text-xs h-7 px-2">
