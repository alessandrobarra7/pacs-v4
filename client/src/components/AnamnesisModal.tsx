...
const BODY_AREAS = [
  ["Cabeça", "left-[42%] top-[7%]"], ["Pescoço", "left-[42%] top-[21%]"], ["Tórax direito", "left-[27%] top-[34%]"],
  ["Tórax esquerdo", "left-[56%] top-[34%]"], ["Abdômen", "left-[42%] top-[46%]"], ["Lombar", "left-[42%] top-[58%]"],
  ["Quadril direito", "left-[29%] top-[67%]"], ["Quadril esquerdo", "left-[55%] top-[67%]"], ["Membro direito", "left-[25%] top-[82%]"], ["Membro esquerdo", "left-[59%] top-[82%]"],
] as const;
    ...
    <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
      ...
      {structuredQuery.isLoading ? <div className="flex justify-center py-12"><Loader2 className="h-7 w-7 animate-spin" /></div> : !modality ? <div className="grid grid-cols-2 gap-3 py-4 md:grid-cols-4">
        ...
        <div className="space-y-4">{questionnaire?.questions.map((question, index) => <div key={question.id} className="rounded-lg border border-slate-200 p-3">
          ...
          {question.id === "pain" && answers.pain === "Sim" && <div className="mt-3 rounded-lg border border-primary/20 bg-primary/5 p-3"><div className="mb-2 flex items-center gap-1 text-xs font-semibold text-primary"><MapPin className="h-3.5 w-3.5" />Marque a região informada pelo paciente</div><div className="relative mx-auto h-64 max-w-xs rounded-md border border-dashed border-slate-300 bg-white"><div className="absolute left-1/2 top-4 h-44 w-20 -translate-x-1/2 rounded-[45%] border-2 border-slate-300 bg-slate-50" /><div className="absolute left-[37%] top-[7%] h-10 w-10 rounded-full border-2 border-slate-300 bg-slate-50" />{BODY_AREAS.map(([area, position]) => <button key={area} type="button" onClick={() => togglePainArea(area)} aria-pressed={painLocations.includes(area)} className={`absolute ${position} h-5 w-5 rounded-full border-2 text-[0px] ${painLocations.includes(area) ? "border-primary bg-primary" : "border-slate-400 bg-white"}`} title={area}>{area}</button>)}</div><div className="mt-2 flex flex-wrap gap-1">{painLocations.map((area) => <span key={area} className="rounded bg-primary/10 px-2 py-0.5 text-xs text-primary">{area}</span>)}</div></div>}
        </div>)}</div>
      </div>}
