...
import { Plus, Pencil, Trash2, ArrowLeft, FileText, AlertTriangle, Search } from "lucide-react";
          ...
          <div className="bg-slate-50/70 p-3 sm:p-4">
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
              {filteredEntries.map((entry: CatalogEntry) => {
                return <article key={entry.id} className="group flex min-h-[148px] flex-col rounded-lg border border-slate-200 bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-md">
                  <div className="flex items-start justify-between gap-2">
                    <h2 className="min-w-0 truncate text-[13px] font-semibold leading-tight text-slate-900" title={entry.exam_name}>{entry.exam_name}</h2>
                    <div className="flex shrink-0 items-center gap-1"><Badge variant="outline" className="h-5 border-cyan-200 bg-cyan-50 px-1.5 text-[10px] text-cyan-800">{entry.modality}</Badge><Badge variant="outline" className={`h-5 px-1.5 text-[10px] ${entry.is_active ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-300 bg-slate-100 text-slate-500"}`}>{entry.is_active ? "Ativo" : "Inativo"}</Badge></div>
                  </div>

                  <div className="mt-3 grid grid-cols-3 overflow-hidden rounded-md border border-slate-100 bg-slate-50">
                  ...
                  <Button variant="outline" size="sm" className="mt-auto h-8 w-full border-slate-300 bg-white text-xs text-slate-700 group-hover:border-cyan-300 group-hover:bg-cyan-50 group-hover:text-cyan-800" onClick={() => startEdit(entry)}><Pencil className="mr-1.5 h-3.5 w-3.5" />Configurar</Button>
                </article>;
              })}
            </div>
          </div>
...
function CatalogMetric({ label, value, accent, divided = false }: { label: string; value: number; accent: string; divided?: boolean }) {
  return <div className={`px-1.5 py-1.5 text-center ${divided ? "border-l border-slate-100" : ""}`}><p className="text-[9px] font-medium uppercase tracking-wide text-slate-500">{label}</p><p className={`text-sm font-semibold ${accent}`}>{value}</p></div>;
}
