...
function UnitCatalogCard({ unit, onOpen }: { unit: UnitSummary; onOpen: () => void }) {
  ...
  return (
    <article className="group flex min-h-[218px] flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-md">
      ...
      <p className="mt-3 text-[11px] leading-4 text-slate-500">A consulta de taxas e valores de períodos anteriores fica na aba Histórico.</p><div className="mt-auto flex justify-end pt-4"><Button size="sm" onClick={onOpen} className="h-8 bg-cyan-700 px-3 text-xs hover:bg-cyan-600">Abrir financeiro <ArrowRight className="ml-1.5 h-3.5 w-3.5" /></Button></div>
    </article>
  );
}
...
function UnitModalityPrices({ unitId, referenceDate, canManage }: { unitId: number; referenceDate: string; canManage: boolean }) {
  ...
  })}</div><div className="flex justify-end border-t border-slate-100 px-5 py-3"><AuditTrailLauncher unitId={unitId} referenceDate={referenceDate} /></div></>;
}
...
function AuditTrailLauncher({ unitId, referenceDate }: { unitId: number; referenceDate: string }) {
  ...
  return <><Button variant="outline" size="sm" className="h-8 border-cyan-200 text-xs text-cyan-800 hover:bg-cyan-50" onClick={() => setOpen(true)}><ScrollText className="mr-1.5 h-3.5 w-3.5" />Ver log do ciclo</Button><Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[88vh] max-w-6xl overflow-hidden p-0"><DialogHeader className="border-b border-slate-100 px-6 py-5"><DialogTitle className="flex items-center gap-2"><ScrollText className="h-5 w-5 text-cyan-700" /> Log auditável dos eventos financeiros</DialogTitle><p className="mt-1 text-sm font-normal text-slate-500">Cada linha identifica a assinatura que gerou o evento, sua ocorrência clínica, valores e situação financeira.</p></DialogHeader><div className="border-b border-slate-100 px-6 py-3"><Input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Buscar por paciente, médico, modalidade ou exame" className="h-9 max-w-md text-sm" /></div><div className="max-h-[60vh] overflow-auto"><table className="min-w-[1080px] w-full text-sm"><thead className="sticky top-0 bg-slate-50 text-[10px] uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-5 py-3 text-left">Data e evento</th><th className="px-5 py-3 text-left">Paciente e estudo</th><th className="px-5 py-3 text-left">Médico da assinatura</th><th className="px-4 py-3 text-left">Origem</th><th className="px-5 py-3 text-right">Médico</th><th className="px-5 py-3 text-right">LAUDS</th><th className="px-5 py-3 text-left">Situação</th></tr></thead><tbody className="divide-y divide-slate-100">{isLoading ? <tr><td colSpan={7} className="py-12 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-cyan-700" /></td></tr> : isError ? <tr><td colSpan={7} className="py-12 text-center text-rose-700"><p className="font-semibold">Falha ao carregar o log auditável.</p><p className="mt-1 text-sm text-rose-600">{errorMessage}</p><Button type="button" variant="outline" size="sm" className="mt-4 border-rose-200 text-rose-700 hover:bg-rose-50" onClick={() => void refetch()}>Tentar novamente</Button></td></tr> : events.length === 0 ? <tr><td colSpan={7} className="py-12 text-center text-slate-500">{filter ? "Nenhum evento encontrado." : "Não há eventos no ciclo consultado."}</td></tr> : events.map((event) => <tr key={event.id} className={`align-top hover:bg-slate-50 ${event.financial_status === "cancelled" ? "bg-rose-50/50" : ""}`}><td className="px-5 py-3"><p className="font-medium text-slate-800">{auditDate(event.signed_at)}</p><p className="mt-0.5 text-xs text-slate-400">{event.modality ?? "—"} · {event.clinical_label ?? event.study_description ?? "Evento financeiro"}</p></td><td className="px-5 py-3"><p className="font-semibold text-slate-900">{event.patient_name?.replace(/\^/g, " ") ?? "Paciente não disponível"}</p><p className="mt-0.5 text-xs text-slate-500">Estudo: {auditDate(event.study_date)}{event.study_description ? ` · ${event.study_description}` : ""}</p></td><td className="px-5 py-3 text-slate-700"><p>{event.doctor_name ?? "Médico não disponível"}</p><p className="mt-1 text-[10px] font-medium uppercase tracking-wide text-slate-400">{event.financial_status === "cancelled" ? "Assinatura histórica cancelada" : "Assinatura registrada no evento"}</p></td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${event.source === "catalog" ? "bg-cyan-50 text-cyan-700" : "bg-slate-100 text-slate-600"}`}>{event.source === "catalog" ? "Catálogo" : "Legado"}</span><p className="mt-2 max-w-44 text-[11px] leading-4 text-slate-500">{auditOriginDescription(event)}</p></td><td className="px-5 py-3 text-right font-semibold text-slate-800">{fmtBRL(asMoney(event.doctor_amount_due))}<p className={`mt-1 text-[10px] font-semibold uppercase ${event.financial_status === "cancelled" ? "text-rose-700" : event.doctor_received_at ? "text-emerald-700" : "text-amber-700"}`}>{event.financial_status === "cancelled" ? "Cancelado" : event.doctor_received_at ? "Pago" : "Pendente"}</p></td><td className="px-5 py-3 text-right font-semibold text-slate-800">{fmtBRL(asMoney(event.system_amount_due))}<p className={`mt-1 text-[10px] font-semibold uppercase ${event.financial_status === "cancelled" ? "text-rose-700" : event.system_paid_at ? "text-emerald-700" : "text-amber-700"}`}>{event.financial_status === "cancelled" ? "Cancelado" : event.system_paid_at ? "Quitado" : "Pendente"}</p></td><td className="px-5 py-3 text-xs text-slate-600">{event.financial_status === "cancelled" ? <><p>Evento cancelado; fora dos totais</p><p className="mt-1 text-[11px] text-rose-700">A linha permanece apenas para auditoria.</p></> : event.pricing_status === "ok" ? "Preço aplicado" : event.pricing_status?.replace(/_/g, " ") ?? "Sem status"}</td></tr>)}</tbody></table></div></DialogContent></Dialog></>;
}
...
function HistoricalSettlement({ unitId, referenceDate, cycleLabel }: { unitId: number; referenceDate: string; cycleLabel: string }) {
  ...
  if (isLoading) return <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-10 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-cyan-700" /><p className="mt-3 text-sm text-slate-500">Calculando o fechamento histórico do período.</p></section>;
  if (isError) return <section className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-6 text-rose-800"><p className="font-semibold">Não foi possível montar o fechamento histórico.</p><p className="mt-1 text-sm">{error instanceof Error ? error.message : "Tente novamente."}</p><Button type="button" variant="outline" size="sm" className="mt-4 border-rose-200 text-rose-700" onClick={() => void refetch()}>Tentar novamente</Button></section>;
  return <section className="mt-6 space-y-5"><div className="flex flex-col gap-2 border-b border-slate-200 pb-4 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-lg font-bold text-slate-950">Fechamento histórico do período</h2><p className="mt-1 text-sm text-slate-500">Laudos, valores e baixas financeiras de {cycleLabel}.</p></div><span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Consulta auditável</span></div><div className="grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Sistema · eventos</p><p className="mt-2 text-3xl font-bold text-slate-950">{activeEvents.length}</p><p className="mt-2 text-xs text-slate-600">{systemPaidEvents.length} pago{systemPaidEvents.length === 1 ? "" : "s"} · {activeEvents.length - systemPaidEvents.length} não pago{activeEvents.length - systemPaidEvents.length === 1 ? "" : "s"}</p></div><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Sistema · valor</p><p className="mt-2 text-3xl font-bold text-slate-950">{fmtBRL(systemAmount)}</p><p className="mt-2 text-xs text-slate-600">Pago: {fmtBRL(systemPaidAmount)} · Em aberto: {fmtBRL(systemAmount - systemPaidAmount)}</p></div><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Situação do sistema</p><p className={`mt-2 text-2xl font-bold ${activeEvents.length > 0 && systemPaidEvents.length === activeEvents.length ? "text-emerald-700" : systemPaidEvents.length > 0 ? "text-amber-700" : "text-rose-700"}`}>{activeEvents.length === 0 ? "Sem eventos" : systemPaidEvents.length === activeEvents.length ? "Pago" : systemPaidEvents.length > 0 ? "Parcial" : "Não pago"}</p><p className="mt-2 text-xs text-slate-600">Baixa registrada no próprio evento financeiro.</p></div></div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 p-5"><h3 className="font-bold text-slate-900">Fechamento por médico</h3><p className="mt-1 text-sm text-slate-500">Exporte a lista dos laudos que compõem cada valor.</p></div><div className="overflow-x-auto"><table className="min-w-[880px] w-full text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-5 py-3 text-left">Médico</th><th className="px-4 py-3 text-center">Exames</th><th className="px-5 py-3 text-right">Valor</th><th className="px-5 py-3 text-center">Pagamento</th><th className="px-5 py-3 text-right">Laudos</th></tr></thead><tbody className="divide-y divide-slate-100">{doctorRows.length === 0 ? <tr><td colSpan={5} className="py-10 text-center text-slate-500">Nenhum evento ativo neste período.</td></tr> : doctorRows.map((doctor) => { const status = doctor.paidCount === doctor.events.length ? "Pago" : doctor.paidCount === 0 ? "Não pago" : "Parcial"; const statusClass = status === "Pago" ? "bg-emerald-50 text-emerald-700" : status === "Parcial" ? "bg-amber-50 text-amber-700" : "bg-rose-50 text-rose-700"; return <tr key={doctor.name} className="hover:bg-slate-50"><td className="px-5 py-4 font-semibold text-slate-900">{doctor.name}</td><td className="px-4 py-4 text-center font-medium text-slate-700">{doctor.events.length}</td><td className="px-5 py-4 text-right"><p className="font-bold text-slate-900">{fmtBRL(doctor.amount)}</p><p className="mt-1 text-xs text-slate-500">Pago {fmtBRL(doctor.paidAmount)} · Aberto {fmtBRL(doctor.pendingAmount)}</p></td><td className="px-5 py-4 text-center"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusClass}`}>{status}</span></td><td className="px-5 py-4 text-right"><Button type="button" variant="outline" size="sm" className="border-cyan-200 text-cyan-800 hover:bg-cyan-50" onClick={() => exportDoctorCsv(doctor)}><FileText className="mr-1.5 h-3.5 w-3.5" />Exportar CSV</Button></td></tr>; })}</tbody></table></div></div></section>;
}
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 md:py-8">
      ...
      <div className="mt-6 flex flex-col gap-3 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {activeView === "history" && <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-1 shadow-sm"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => changeHistoryMonth(-1)}><ChevronLeft className="h-4 w-4" /></Button><span className="min-w-40 text-center text-sm font-semibold text-slate-800"><CalendarDays className="mr-1.5 inline h-4 w-4 text-cyan-700" />{MONTHS[historyMonth - 1]} {historyYear}</span><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => changeHistoryMonth(1)}><ChevronRight className="h-4 w-4" /></Button></div>}
      </div>

      {activeView === "current" ? <section className="mt-6"><div className="rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4 text-sm text-cyan-950"><CalendarDays className="mr-2 inline h-4 w-4 text-cyan-700" /><strong>Ciclo operacional atual:</strong> {currentUnit.cycle_label}. Os três indicadores abaixo usam somente eventos pertencentes a este ciclo.</div><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-cyan-100 bg-cyan-50/50 p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-cyan-700">Valor configurado por evento</p><p className="mt-2 text-3xl font-bold text-slate-950">{defaultPrices?.default_system_price == null ? "Não definida" : fmtBRL(asMoney(defaultPrices.default_system_price))}</p><p className="mt-2 text-xs text-cyan-800">Taxa vigente para a próxima assinatura financeira.</p>{isAdminMaster && <div className="mt-3"><SystemRateEditor unitId={currentUnit.unit_id} currentPrice={defaultPrices?.default_system_price} defaultDoctorPrice={defaultPrices?.default_doctor_price} /></div>}</div><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Eventos do ciclo atual</p><p className="mt-2 text-3xl font-bold text-slate-950">{currentUnit.total_laudos}</p><p className="mt-2 text-xs text-slate-600">Assinaturas financeiras registradas entre {currentUnit.cycle_label}.</p></div><div className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-emerald-700">Rendimento atual do ciclo</p><p className="mt-2 text-3xl font-bold text-slate-950">{fmtBRL(asMoney(currentUnit.system_total))}</p><p className="mt-2 text-xs text-emerald-800">Total já gerado pelos {currentUnit.total_laudos} evento{currentUnit.total_laudos === 1 ? "" : "s"} deste ciclo.</p></div></div></section> : <section className="mt-6"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700"><ScrollText className="mr-2 inline h-4 w-4 text-slate-600" /><strong>Consulta histórica:</strong> este período não altera a configuração atual da unidade e não participa do painel operacional.</div><div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Eventos do período</p><p className="mt-2 text-3xl font-bold text-slate-950">{historicalUnit.total_laudos}</p><p className="mt-2 text-xs text-slate-600">{historicalUnit.cycle_label}</p></div><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Taxas registradas</p><p className="mt-2 text-base font-bold text-slate-950">{historicalRates}</p><p className="mt-2 text-xs text-slate-600">Snapshots gravados quando cada evento foi assinado.</p></div><div className="rounded-xl border border-slate-200 bg-white p-5"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Rendimento histórico</p><p className="mt-2 text-3xl font-bold text-slate-950">{fmtBRL(systemCycleTotal)}</p><p className="mt-2 text-xs text-slate-600">Soma preservada do período consultado.</p></div></div></section>}
      ...
      {activeView === "current" && <><section className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-bold text-slate-900">Valores vigentes por modalidade</h2><p className="mt-1 text-sm text-slate-500">Configuração para novas assinaturas; não altera eventos de ciclos anteriores.</p></div><span className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500"><Settings2 className="h-3.5 w-3.5" /> Configuração atual da unidade</span></div><UnitModalityPrices unitId={currentUnit.unit_id} referenceDate={currentReference} canManage={canManagePrices} /></section><section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-2 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-bold text-slate-900">Médicos no ciclo atual</h2><p className="mt-1 text-sm text-slate-500">Eventos e rendimento calculados exclusivamente para {currentUnit.cycle_label}.</p></div><span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-600"><Stethoscope className="h-4 w-4 text-cyan-700" /> {rows.length} médico{rows.length === 1 ? "" : "s"}</span></div><div className="overflow-x-auto"><table className="min-w-[760px] w-full text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-5 py-3 text-left">Médico</th>{MODALITIES.map((modality) => <th key={modality} className="px-2 py-3 text-center">{MODALITY_LABEL[modality]}</th>)}<th className="px-4 py-3 text-center">Eventos</th><th className="px-5 py-3 text-right">Total no ciclo</th></tr></thead><tbody className="divide-y divide-slate-100">{doctorsLoading ? <tr><td colSpan={8} className="py-10 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-cyan-700" /></td></tr> : rows.length === 0 ? <tr><td colSpan={8} className="py-10 text-center text-slate-500">Nenhum médico vinculado a esta unidade.</td></tr> : rows.map((doctor) => <tr key={doctor.id} className="hover:bg-slate-50/80"><td className="px-5 py-3 font-semibold text-slate-900">{doctor.name}</td><DoctorModalityCells unitId={currentUnit.unit_id} doctorId={doctor.id} responsibleId={readiness?.responsible_id ?? null} fallbacks={fallbacks} canManage={canManagePrices} /><td className="px-4 py-3 text-center font-medium text-slate-600">{doctor.summary?.total_laudos ?? 0}</td><td className="px-5 py-3 text-right font-bold text-slate-900">{fmtBRL(asMoney(doctor.summary?.doctor_total))}</td></tr>)}</tbody></table></div></section></>}
    </div>
  );
}
...
export default function FinanceDashboard() {
  ...
  if (isUnitRoute && currentUnit && historicalUnit) return <div className="min-h-screen bg-[#f6f8fb] text-slate-900"><AppHeader nav={nav} /><UnitFinancialDetail currentUnit={currentUnit} historicalUnit={historicalUnit} activeView={activeView} onChangeView={setActiveView} historyYear={historyYear} historyMonth={historyMonth} onChangeHistoryMonth={changeHistoryMonth} onBack={() => navigate("/financeiro/dashboard")} /></div>;
  return <div className="min-h-screen bg-[#f6f8fb] text-slate-900"><AppHeader nav={nav} /><main className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 md:py-8"><div className="flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-cyan-700">Administração financeira</p><h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-950">Ciclos financeiros atuais</h1><p className="mt-1 text-sm text-slate-500">Acompanhe apenas o ciclo atualmente aberto de cada unidade. A consulta de períodos anteriores fica dentro de cada unidade.</p></div><span className="inline-flex items-center gap-2 rounded-xl border border-cyan-100 bg-cyan-50 px-3 py-2 text-sm font-semibold text-cyan-900"><CalendarDays className="h-4 w-4 text-cyan-700" /> Atualizado para o ciclo em aberto</span></div><div className="mt-6 rounded-xl border border-cyan-100 bg-cyan-50/70 px-4 py-3 text-sm text-cyan-950"><FileText className="mr-2 inline h-4 w-4 text-cyan-700" />Cada cartão apresenta o valor configurado para novos eventos, a quantidade de eventos e o rendimento do ciclo que está aberto agora.</div>{currentLoading ? <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{[1, 2, 3, 4, 5, 6].map((key) => <div key={key} className="h-56 animate-pulse rounded-2xl bg-slate-200" />)}</div> : currentUnits.length === 0 ? <div className="mt-12 rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center"><Building2 className="mx-auto h-10 w-10 text-slate-300" /><h2 className="mt-4 text-base font-semibold text-slate-800">Nenhuma unidade configurada</h2><p className="mt-1 text-sm text-slate-500">As unidades aparecerão aqui mesmo antes do primeiro evento do ciclo atual.</p></div> : <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{(currentUnits as UnitSummary[]).map((unit) => <UnitCatalogCard key={unit.unit_id} unit={unit} onOpen={() => navigate(`/financeiro/dashboard/${unitSlug(unit.unit_name)}`)} />)}</div>}</main></div>;
}
