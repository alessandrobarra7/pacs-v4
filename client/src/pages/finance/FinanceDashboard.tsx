...
function UnitCatalogCard({ unit, onOpen }: { unit: UnitSummary; onOpen: () => void }) {
  ...
  return (
    <article className="group flex min-h-[218px] flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-md">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2"><MiniMetric label="Eventos no ciclo" value={String(eventCount)} tone="cyan" /><MiniMetric label="Taxa para novos eventos" value={prices?.default_system_price == null ? "Não definida" : fmtBRL(systemRate)} tone="emerald" /><MiniMetric label="Total histórico do ciclo" value={fmtBRL(systemCycleTotal)} tone="emerald" /></div>
      <p className="mt-3 text-[11px] leading-4 text-slate-500">Taxas aplicadas no ciclo: {historicalSystemRatesLabel(unit)}</p><div className="mt-auto flex justify-end pt-4"><Button size="sm" onClick={onOpen} className="h-8 bg-cyan-700 px-3 text-xs hover:bg-cyan-600">Abrir financeiro <ArrowRight className="ml-1.5 h-3.5 w-3.5" /></Button></div>
    </article>
  );
}
...
function UnitFinancialDetail({ unit, year, month, onBack }: { unit: UnitSummary; year: number; month: number; onBack: () => void }) {
  ...
  const systemCycleTotal = asMoney(unit.system_total);
  const historicalRates = historicalSystemRatesLabel(unit);
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-6 md:py-8"><button onClick={onBack} className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-slate-600 transition-colors hover:text-cyan-800"><ChevronLeft className="h-4 w-4" /> Voltar às unidades</button><div className="flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-start md:justify-between"><div><div className="flex items-center gap-2"><span className="text-xs font-semibold uppercase tracking-[0.15em] text-cyan-700">Financeiro / Unidades</span></div><h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-950">{unit.unit_name}</h1><p className="mt-1 text-sm text-slate-500">Ciclo consultado: {unit.cycle_label}. As configurações atuais valem para novas assinaturas; os totais preservam o valor aplicado em cada evento.</p></div><span className="inline-flex w-fit items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200"><CheckCircle2 className="h-3.5 w-3.5" /> Unidade ativa</span></div><section className="mt-6 grid gap-3 md:grid-cols-3"><div className="rounded-xl border border-cyan-100 bg-cyan-50/50 p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-cyan-700">Taxa para novas assinaturas</p><p className="mt-2 text-2xl font-bold text-slate-950">{defaultPrices?.default_system_price == null ? "Não definida" : fmtBRL(asMoney(defaultPrices.default_system_price))}</p><p className="mt-1 text-xs text-cyan-800">Vigente agora; não recalcula eventos já assinados.</p>{isAdminMaster && <div className="mt-3"><SystemRateEditor unitId={unit.unit_id} currentPrice={defaultPrices?.default_system_price} defaultDoctorPrice={defaultPrices?.default_doctor_price} /></div>}</div><div className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Eventos no ciclo</p><p className="mt-2 text-2xl font-bold text-slate-950">{unit.total_laudos}</p><p className="mt-1 text-xs font-medium text-slate-600">Período: {unit.cycle_label}</p></div><div className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-emerald-700">Total histórico do ciclo</p><p className="mt-2 text-2xl font-bold text-slate-950">{fmtBRL(systemCycleTotal)}</p><p className="mt-1 text-xs text-emerald-700">Taxas preservadas nos {unit.total_laudos} evento{unit.total_laudos === 1 ? "" : "s"}: {historicalRates}</p></div></section><section className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-bold text-slate-900">Valor vigente por modalidade</h2><p className="mt-1 text-sm text-slate-500">Valor padrão da unidade; médicos sem valor individual recebem este fallback.</p></div><span className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500"><Settings2 className="h-3.5 w-3.5" /> Configuração financeira da unidade</span></div><UnitModalityPrices unitId={unit.unit_id} referenceDate={referenceDate} canManage={canManagePrices} /></section><section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-2 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-bold text-slate-900">Médicos envolvidos — preços por modalidade</h2><p className="mt-1 text-sm text-slate-500">Edite diretamente as células. Um campo vazio usa o valor padrão vigente da unidade.</p></div><span className="inline-flex items-center gap-2 text-xs font-semibold text-slate-600"><Stethoscope className="h-4 w-4 text-cyan-700" /> {rows.length} médico{rows.length === 1 ? "" : "s"}</span></div><div className="overflow-x-auto"><table className="min-w-[760px] w-full text-sm"><thead className="bg-slate-50 text-[11px] uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-5 py-3 text-left">Médico</th>{MODALITIES.map((modality) => <th key={modality} className="px-2 py-3 text-center">{MODALITY_LABEL[modality]}</th>)}<th className="px-4 py-3 text-center">Eventos</th><th className="px-5 py-3 text-right">Total no ciclo</th></tr></thead><tbody className="divide-y divide-slate-100">{doctorsLoading ? <tr><td colSpan={8} className="py-10 text-center"><Loader2 className="mx-auto h-5 w-5 animate-spin text-cyan-700" /></td></tr> : rows.length === 0 ? <tr><td colSpan={8} className="py-10 text-center text-slate-500">Nenhum médico vinculado a esta unidade.</td></tr> : rows.map((doctor) => <tr key={doctor.id} className="hover:bg-slate-50/80"><td className="px-5 py-3 font-semibold text-slate-900">{doctor.name}</td><DoctorModalityCells unitId={unit.unit_id} doctorId={doctor.id} responsibleId={readiness?.responsible_id ?? null} fallbacks={fallbacks} canManage={canManagePrices} /><td className="px-4 py-3 text-center font-medium text-slate-600">{doctor.summary?.total_laudos ?? 0}</td><td className="px-5 py-3 text-right font-bold text-slate-900">{fmtBRL(asMoney(doctor.summary?.doctor_total))}</td></tr>)}</tbody></table></div></section></div>;
}
